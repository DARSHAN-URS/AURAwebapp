  ...
  return (
    <div className="flex flex-col gap-6 w-full select-none">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {/* Progress gauge */}
        <div className="bg-card border border-border rounded-3xl p-5 shadow-sm flex flex-col justify-between min-h-[120px]">
          <span className="text-[10px] font-black uppercase text-muted-text tracking-wider">Overall Progress</span>
          <div className="my-2">
            <span className="text-2xl font-black text-primary leading-none">{stats.overall_progress}%</span>
          ...
          <span className="text-[9px] text-muted-text font-bold block">Target: Completed Arrival</span>
        </div>

        {/* Health Index */}
        <div className="bg-card border border-border rounded-3xl p-5 shadow-sm flex flex-col justify-between min-h-[120px]">
          <span className="text-[10px] font-black uppercase text-muted-text tracking-wider">Journey Health</span>
          <div className="my-2 flex items-center justify-between">
            <span className="text-2xl font-black text-foreground leading-none">{stats.health_score}%</span>
          ...
          <span className="text-[9px] text-muted-text font-bold block">Overdue check: None</span>
        </div>

        {/* Task complete ratios */}
        <div className="bg-card border border-border rounded-3xl p-5 shadow-sm flex flex-col justify-between min-h-[120px]">
          <span className="text-[10px] font-black uppercase text-muted-text tracking-wider">Milestones Completed</span>
          <div className="my-2">
            <span className="text-2xl font-black text-foreground leading-none">
              {stats.completed_tasks} / {stats.total_tasks}
            </span>
          </div>
          <span className="text-[9px] text-muted-text font-bold block">
        ...
        <div className="bg-card border border-border rounded-3xl p-5 shadow-sm flex flex-col justify-between min-h-[120px]">
          <span className="text-[10px] font-black uppercase text-muted-text tracking-wider">Visa Readiness</span>
          <div className="my-2 flex items-center justify-between">
          ...
          <span className="text-[9px] text-muted-text font-bold block">Checked with AI Auditor</span>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-3xl p-6 sm:p-8 shadow-sm flex flex-col gap-5">
        <div>
          <h3 className="text-xs font-black uppercase text-muted-text tracking-wider flex items-center gap-1.5 select-none">
            <Lightbulb className="w-4.5 h-4.5 text-amber-500 fill-amber-50" />
            Aura AI Recommendations
          </h3>
          <p className="text-[11px] text-muted-text mt-0.5 leading-snug">Personalized study-abroad action plans driven by real-time milestone logs.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {recommendations.map((rec, idx) => (
            <div 
              key={idx}
              className="bg-background/50 hover:bg-background border border-border p-4 rounded-2xl flex flex-col justify-between gap-4 transition-all"
            >
              <div>
                ...
                <p className="text-[11px] text-muted-foreground mt-1 leading-relaxed leading-normal">{rec.desc}</p>
              </div>

              <Button
                variant="link"
                onClick={() => onTabNavigate(rec.tab)}
                className="text-[10px] font-black text-primary hover:text-primary p-0 h-auto self-start flex items-center gap-0.5 cursor-pointer no-underline hover:underline"
              >
