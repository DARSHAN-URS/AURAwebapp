...
export default function EligibilityResultPage() {
  ...
  if (loading) {
    return (
      <div className="bg-card min-h-screen pt-32 pb-24 flex justify-center">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
          {/* Skeleton Title */}
          <div className="h-8 bg-muted rounded-full w-2/3 mb-4 animate-pulse mx-auto" />
          <div className="h-4 bg-background rounded-full w-1/3 mb-16 animate-pulse mx-auto" />

          <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start">
            {/* Score Ring Skeleton */}
            <div className="md:col-span-4 bg-card border border-border rounded-3xl p-8 shadow-sm flex flex-col items-center gap-4 animate-pulse">
              <div className="w-36 h-36 rounded-full border-8 border-border flex items-center justify-center" />
              <div className="h-6 bg-muted rounded-full w-2/3" />
              <div className="h-4 bg-background rounded-full w-1/2" />
            </div>

            {/* Metrics List Skeleton */}
            <div className="md:col-span-8 bg-card border border-border rounded-3xl p-8 shadow-sm flex flex-col gap-6 animate-pulse">
              <div className="h-5 bg-muted rounded-full w-1/4" />
              <div className="h-10 bg-background rounded-xl w-full" />
              <div className="h-10 bg-background rounded-xl w-full" />
              <div className="h-10 bg-background rounded-xl w-full" />
            </div>
          </div>
    ...
    return (
      <div className="bg-card min-h-screen pt-32 pb-24 flex items-center justify-center">
        <div className="max-w-md w-full px-6 text-center">
          ...
          <h2 className="text-xl font-bold text-foreground mb-2">Evaluation Loading Failed</h2>
          <p className="text-sm text-muted-foreground leading-relaxed mb-8">{error || "Data is unavailable."}</p>
          <div className="flex gap-4 justify-center">
            <Button onClick={handleRetry} className="bg-primary hover:bg-primary text-white font-bold rounded-full px-6 cursor-pointer">
              Start New Check
            </Button>
            <Button onClick={() => window.location.reload()} variant="outline" className="border-border text-foreground/80 rounded-full px-6 cursor-pointer">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-sm text-muted-text mt-2 font-medium">
            Student: <strong className="text-foreground">{request.full_name}</strong> • Reference: {request.id.slice(0, 8)}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start mb-12">
          ...
          <div className="lg:col-span-4 bg-card border border-border rounded-3xl p-8 shadow-lg flex flex-col items-center text-center relative overflow-hidden">
            <div className="absolute top-0 left-0 w-full h-1.5 bg-primary" />
            
            <h3 className="font-extrabold text-foreground text-sm uppercase tracking-wider mb-6">Overall Score</h3>
            ...
            <div className="relative w-36 h-36 flex items-center justify-center mb-6">
              ...
              <div className="absolute flex flex-col items-center">
                ...
                <span className="text-[10px] font-bold text-muted-text uppercase tracking-widest mt-0.5">Scale 100</span>
              </div>
            </div>
            ...
            <p className="text-xs text-muted-text leading-relaxed mt-6 border-t border-gray-50 pt-6">
              Evaluated against university criteria, IELTS thresholds, and financial limits in <span className="font-bold text-muted-foreground capitalize">{request.preferred_country}</span>.
            </p>
          </div>

          {/* Right Column: Key metrics assessment */}
          <div className="lg:col-span-8 bg-card border border-border rounded-3xl p-8 shadow-lg flex flex-col justify-between h-full min-h-[320px]">
            <div>
              <h3 className="font-extrabold text-foreground text-sm uppercase tracking-wider mb-8">AI Evaluation Indexes</h3>
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between items-center text-xs font-bold text-muted-foreground uppercase">
                    ...
                    <span className="text-foreground">{result.admission_probability}</span>
                  </div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between items-center text-xs font-bold text-muted-foreground uppercase">
                    ...
                    <span className="text-foreground">{result.scholarship_potential}</span>
                  </div>
                  <div className="h-2 bg-muted rounded-full overflow-hidden">
                    <div 
                      className={`h-full rounded-full transition-all duration-1000 ${
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex justify-between items-center text-xs font-bold text-muted-foreground uppercase">
                    ...
                    <span className="text-foreground">{result.visa_readiness}</span>
                  </div>
                  <div className="h-2 bg-muted rounded-full overflow-hidden">
                    <div 
                      className={`h-full rounded-full transition-all duration-1000 ${
            ...
            <div className="flex items-center gap-2 mt-8 pt-6 border-t border-gray-50 text-[11px] text-muted-text font-medium">
        ...
        <div className="bg-card border border-border rounded-3xl p-8 shadow-lg mb-12">
          <h3 className="font-extrabold text-foreground text-sm uppercase tracking-wider mb-6 flex items-center gap-2">
            <Sparkles className="w-5 h-5 text-primary fill-blue-50" />
        ...
        <div className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {result.recommended_universities.map((univ, idx) => (
              <div key={idx} className="bg-card border border-border p-6 rounded-2xl shadow-sm hover:shadow-md hover:border-border transition-all duration-300 flex flex-col justify-between min-h-[180px]">
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    <MapPin className="w-4 h-4 text-primary" />
                    <span className="text-xs font-bold text-muted-text capitalize">{univ.location}</span>
                  </div>
                  <h4 className="font-bold text-foreground text-base mb-2">{univ.name}</h4>
                  <p className="text-xs text-muted-foreground leading-relaxed">{univ.reasoning}</p>
                </div>
                
                <span className="text-[10px] font-bold text-primary bg-primary/10 px-2 py-0.5 rounded-full w-fit mt-4">
                  Match Suggestion
                </span>
              </div>
            ))}
        ...
        <div className="bg-background border border-border rounded-3xl p-8 sm:p-10 mb-12">
          <h3 className="font-extrabold text-foreground text-sm uppercase tracking-wider mb-8 flex items-center gap-2">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-3xl p-8 sm:p-12 text-center shadow-xl flex flex-col items-center relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-xl">
            <div className="w-10 h-10 rounded-full bg-card/10 flex items-center justify-center mb-6 mx-auto">
            ...
            <Button
              onClick={openBooking}
              className="bg-card hover:bg-background text-primary font-bold px-8 py-3.5 rounded-full shadow-lg hover:scale-105 active:scale-95 transition-all duration-200 cursor-pointer text-sm flex items-center gap-2 mx-auto"
            >
