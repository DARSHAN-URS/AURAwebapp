...
export default function SOPEditorWorkspace() {
    ...
    return (
      <div className="bg-card min-h-screen pt-32 pb-24 flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-semibold text-muted-foreground">Opening editor workspace...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-card min-h-screen pt-24 pb-24">
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl no-print">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 border-b border-border pb-4">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <Button
              variant="outline"
              onClick={handleCopy}
              className="h-9 px-3 rounded-xl border-border text-foreground/80 hover:bg-background cursor-pointer text-xs flex items-center gap-1"
            >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-3 bg-background border border-border rounded-3xl p-5 min-h-[300px]">
            <h3 className="font-extrabold text-foreground text-xs uppercase tracking-wider mb-6 flex items-center gap-1.5 border-b border-border/50 pb-2">
            ...
            <div className="flex flex-col gap-4">
              {document?.versions && document.versions.map((ver, idx) => (
                <div 
                  ...
                  className="bg-card border border-border hover:border-primary/20 p-4 rounded-xl shadow-sm hover:shadow-md cursor-pointer transition-all flex flex-col gap-1.5"
                >
                  <div className="flex justify-between items-center text-[10px] font-bold text-primary">
                    <span>Draft #{ver.version_number}</span>
                    <span className="text-muted-text font-semibold">{new Date(ver.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                  </div>
              ...
              {(!document?.versions || document.versions.length === 0) && (
                <p className="text-xs text-muted-text text-center py-6">No previous versions registered.</p>
              )}
            </div>
          </div>

          {/* CENTER PANEL: Notion Rich Editor */}
          <div className="lg:col-span-6 bg-card border border-border rounded-3xl p-6 sm:p-10 shadow-lg min-h-[600px] flex flex-col justify-between" id="print-workspace">
            <div>
              {/* Header Title Editor */}
              <input
                type="text"
                ...
                className="w-full text-2xl font-black text-foreground focus:outline-none border-b border-transparent focus:border-border pb-3 mb-6 no-print"
              ...
              <div className="flex items-center gap-1.5 border-b border-gray-50 pb-3 mb-6 no-print">
                <Button variant="ghost" onClick={() => execCommand("bold")} className="h-8 w-8 p-0 rounded-lg text-muted-foreground hover:text-foreground cursor-pointer" title="Bold"><Bold className="w-4 h-4" /></Button>
                <Button variant="ghost" onClick={() => execCommand("italic")} className="h-8 w-8 p-0 rounded-lg text-muted-foreground hover:text-foreground cursor-pointer" title="Italic"><Italic className="w-4 h-4" /></Button>
                <Button variant="ghost" onClick={() => execCommand("underline")} className="h-8 w-8 p-0 rounded-lg text-muted-foreground hover:text-foreground cursor-pointer" title="Underline"><Underline className="w-4 h-4" /></Button>
                <Button variant="ghost" onClick={() => execCommand("formatBlock", "h2")} className="h-8 w-8 p-0 rounded-lg text-muted-foreground hover:text-foreground cursor-pointer" title="Header"><Heading2 className="w-4 h-4" /></Button>
                <Button variant="ghost" onClick={() => execCommand("insertUnorderedList")} className="h-8 w-8 p-0 rounded-lg text-muted-foreground hover:text-foreground cursor-pointer" title="List"><List className="w-4 h-4" /></Button>
                <div className="w-px h-5 bg-muted/60 mx-2" />
                <Button variant="ghost" onClick={() => execCommand("undo")} className="h-8 w-8 p-0 rounded-lg text-muted-foreground hover:text-foreground cursor-pointer" title="Undo"><Undo2 className="w-4 h-4" /></Button>
                <Button variant="ghost" onClick={() => execCommand("redo")} className="h-8 w-8 p-0 rounded-lg text-muted-foreground hover:text-foreground cursor-pointer" title="Redo"><Redo2 className="w-4 h-4" /></Button>
              </div>
              ...
              <div 
                ...
                className="focus:outline-none min-h-[420px] text-sm sm:text-base text-foreground/90 leading-relaxed font-medium prose max-w-none"
                dangerouslySetInnerHTML={{ __html: editorContent }}
              />
            </div>

            {/* Counts Footer */}
            <div className="flex items-center justify-between border-t border-border pt-6 mt-8 text-xs text-muted-text font-semibold no-print">
          ...
          <div className="lg:col-span-3 flex flex-col gap-6">
            ...
            <div className="bg-card border border-border rounded-3xl p-5 shadow-sm">
              <h3 className="font-extrabold text-foreground text-xs uppercase tracking-wider mb-4 flex items-center gap-1.5 border-b border-gray-50 pb-2">
              ...
              <div className="flex flex-col gap-4">
                ...
                <div>
                  <div className="flex justify-between items-center text-[10px] font-bold text-muted-foreground uppercase mb-1">
                    ...
                    <span className="text-foreground">Grade 11</span>
                  </div>
                  <div className="h-1.5 bg-muted rounded-full overflow-hidden">
            ...
            <div className="bg-background border border-border rounded-3xl p-5 min-h-[220px] flex flex-col justify-between">
              <div>
                <h3 className="font-extrabold text-foreground text-xs uppercase tracking-wider mb-4 flex items-center gap-1.5 border-b border-border/50 pb-2">
                ...
                {selectedText ? (
                  <div className="flex flex-col gap-3.5">
                    <div className="p-3 bg-card border border-border rounded-xl max-h-24 overflow-y-auto">
                      <p className="text-[10px] font-semibold text-muted-text uppercase mb-1">Selected text segment:</p>
                      <p className="text-[11px] text-foreground/80 italic leading-snug">"{selectedText}"</p>
                    </div>
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      <button 
                        ...
                        disabled={aiWorking}
                        className="text-left text-[11px] font-bold text-foreground/80 hover:text-primary px-3 py-1.5 rounded-lg hover:bg-card border border-transparent hover:border-border transition-all flex items-center justify-between cursor-pointer"
                      >
                      ...
                      <div className="grid grid-cols-2 gap-2 mt-1">
                        <Button 
                          ...
                          disabled={aiWorking}
                          className="h-8 rounded-lg bg-card border border-border text-foreground/80 text-[10px] font-bold hover:bg-background cursor-pointer"
                        >
                          Shorten
                        </Button>
                        <Button 
                  ...
                  <div className="text-center py-8">
                    <FileSignature className="w-8 h-8 text-muted-text mx-auto mb-3" />
                    <p className="text-xs text-muted-text max-w-xs leading-relaxed mx-auto">
              ...
              {aiWorking && (
                <div className="mt-4 p-3 bg-card border border-border rounded-xl flex items-center justify-center gap-2 text-xs font-semibold text-primary">
